 ...
 * @version 1.0
 */
AVErrCode OH_AudioDecoder_Destroy(AVCodec *codec);
 ...
 * @version 1.0
 */
AVErrCode OH_AudioDecoder_SetCallback(AVCodec *codec, AVCodecAsyncCallback callback, void *userData);
 ...
 * @syscap SystemCapability.Multimedia.Media.AudioDecoder
 * @param codec pointer to an AVCodec instance
 * @param format A pointer to an AVFormat giving a description of the audio track to be decoded
 * @return Returns AV_ERR_OK if the execution is successful,
 * otherwise returns a specific error code, refer to {@link AVErrCode}
 * @since 9
 * @version 1.0
 */
AVErrCode OH_AudioDecoder_Configure(AVCodec *codec, AVFormat *format);
 ...
 * @version 1.0
 */
AVErrCode OH_AudioDecoder_Prepare(AVCodec *codec);
 ...
 * @version 1.0
 */
AVErrCode OH_AudioDecoder_Start(AVCodec *codec);

/**
 * @brief Stop the decoder. After stopping, you can re-enter the Started state through Start,
 * but it should be noted that if the decoder has been input before
 ...
 * @version 1.0
 */
AVErrCode OH_AudioDecoder_Stop(AVCodec *codec);
 ...
 * @version 1.0
 */
AVErrCode OH_AudioDecoder_Flush(AVCodec *codec);
...
AVErrCode OH_AudioDecoder_Reset(AVCodec *codec);
 ...
 * @version 1.0
 */
AVErrCode OH_AudioDecoder_SetParameter(AVCodec *codec, AVFormat *format);

/**
 * @brief Submit the input buffer filled with data to the audio decoder. The {@link AVCodecOnNeedInputData} callback
 * will report the available input buffer and the corresponding index value. Once the buffer with the specified index
 * is submitted to the audio decoder, the buffer cannot be accessed again until the {@link AVCodecOnNeedInputData}
 ...
 * @version 1.0
 */
AVErrCode OH_AudioDecoder_PushInputData(AVCodec *codec, uint32_t index, AVCodecBufferAttr attr);
 ...
 * @version 1.0
 */
AVErrCode OH_AudioDecoder_FreeOutputData(AVCodec *codec, uint32_t index);
