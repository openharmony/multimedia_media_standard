...
int32_t RecorderServiceStub::SetListenerObject(const sptr<IRemoteObject> &object)
{
    ...
    deathRecipient->SetNotifyCb(std::bind(&RecorderServiceStub::ClientDied, this));

    std::shared_ptr<RecorderCallback> callback = std::make_shared<RecorderListenerCallback>(listener, deathRecipient);
    ...
    recorderServer_->SetRecorderCallback(callback);
    return ERR_OK;
}
