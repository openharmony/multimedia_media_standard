...
std::shared_ptr<IRecorderService> RecorderServer::Create()
{
    ...
    if (server == nullptr) {
        MEDIA_LOGE("failed to new RecorderServer");
        return nullptr;
    }
...
int32_t RecorderServer::Init()
{
    ...
    recorderEngine_ = RecorderEngineDl::Instance().CreateEngine();
    CHECK_AND_RETURN_RET_LOG(recorderEngine_ != nullptr, ERR_NO_INIT, "failed to create recorder engine");
...
void RecorderServer::OnError(ErrorType errorType, int32_t errorCode)
{
    ...
    if (recorderCb_ != nullptr) {
        recorderCb_->OnError(static_cast<int32_t>(errorType), errorCode);
    }
}
...
void RecorderServer::OnInfo(InfoType type, int32_t extra)
{
    std::lock_guard<std::mutex> lock(mutex_);
    if (recorderCb_ != nullptr) {
        recorderCb_->OnInfo(static_cast<int32_t>(type), extra);
    }
}
...
int32_t RecorderServer::SetVideoSource(VideoSourceType source, int32_t &sourceId)
{
    ...
    CHECK_AND_RETURN_RET_LOG(recorderEngine_ != nullptr, ERR_INVALID_OPERATION, "engine is nullptr");
...
{
    std::lock_guard<std::mutex> lock(mutex_);
    if (status_ != REC_PREPARED && status_ != REC_RECORDING && status_ != REC_PAUSED) {
        MEDIA_LOGE("invalid status, current status is %{public}d", status_);
        return nullptr;
    }
...
int32_t RecorderServer::SetMaxDuration(int32_t duration)
{
    ...
    return recorderEngine_->Configure(0, maxDuration);
}
...
int32_t RecorderServer::SetOutputFormat(OutputFormatType format)
{
    ...
    status_ = (ret == ERR_OK ? REC_CONFIGURED : REC_INITIALIZED);
    return ret;
}
...
int32_t RecorderServer::SetOutputPath(const std::string &path)
{
    ...
    return recorderEngine_->Configure(0, outFilePath);
}
...
int32_t RecorderServer::SetOutputFile(int32_t fd)
{
    ...
    CHECK_AND_RETURN_RET_LOG(recorderEngine_ != nullptr, ERR_INVALID_OPERATION, "engine is nullptr");
    OutFd outFileFd(fd);
    return recorderEngine_->Configure(0, outFileFd);
}
...
int32_t RecorderServer::SetNextOutputFile(int32_t fd)
{
    ...
    return recorderEngine_->Configure(0, nextFileFd);
}
...
int32_t RecorderServer::SetMaxFileSize(int64_t size)
{
    ...
    return recorderEngine_->Configure(0, maxFileSize);
}

int32_t RecorderServer::SetRecorderCallback(const std::shared_ptr<RecorderCallback> &callback)
{
    std::lock_guard<std::mutex> lock(mutex_);
    if (status_ != REC_INITIALIZED && status_ != REC_CONFIGURED) {
        MEDIA_LOGE("invalid status, current status is %{public}d", status_);
        return ERR_INVALID_OPERATION;
    }
...
int32_t RecorderServer::Prepare()
{
    ...
    status_ = (ret == ERR_OK ? REC_PREPARED : REC_ERROR);
    return ret;
}
...
int32_t RecorderServer::Start()
{
    ...
    status_ = (ret == ERR_OK ? REC_RECORDING : REC_ERROR);
...
int32_t RecorderServer::Pause()
{
    ...
    status_ = (ret == ERR_OK ? REC_PAUSED : REC_ERROR);
    return ret;
}
...
{
    std::lock_guard<std::mutex> lock(mutex_);
    if (status_ != REC_RECORDING && status_ != REC_PAUSED) {
        MEDIA_LOGE("invalid status, current status is %{public}d", status_);
        return ERR_INVALID_OPERATION;
    }
    CHECK_AND_RETURN_RET_LOG(recorderEngine_ != nullptr, ERR_INVALID_OPERATION, "engine is nullptr");
...
int32_t RecorderServer::Stop(bool block)
{
    ...
    status_ = (ret == ERR_OK ? REC_INITIALIZED : REC_ERROR);
    return ret;
}
