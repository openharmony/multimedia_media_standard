...
#define ADD_LINK_DESC(srcElem, dstElem, srcPad, sinkPad, isSrcPadStatic, isSinkPadStatic)                       \
    do {                                                                                                        \
        ...
        if (pipelineDesc_->allLinkDescs.count(srcElem) == 0) {                                                   \
            pipelineDesc_->allLinkDescs.emplace(srcElem, std::vector<RecorderPipelineDesc::LinkDesc>());         \
        }                                                                                                       \
        pipelineDesc_->allLinkDescs[srcElem].push_back(linkDesc);                                                \
    } while (false)
...
int32_t RecorderPipelineBuilder::SetAudioSource(const RecorderSourceDesc &desc)
{
    ...
    ADD_LINK_DESC(audioSrcElem, audioEncElem, "src", "sink", true, true);
