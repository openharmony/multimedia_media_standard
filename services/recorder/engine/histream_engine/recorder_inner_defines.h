...
struct RecorderSourceDesc {
    ...
    /**
      Handle is currently represented as int32_t, and internel descripted as source kind mask + index :
...
/**
 * @brief Recorder engine internel upward notification type.
 */
enum RecorderNotifyType : int32_t {
    RECORDER_NOTIFY_ERROR,
    RECORDER_NOTIFY_INFO,
};

/**
 * @brief Recorder engine internel upward notification error type.
 */
enum RecorderPrivateErrorType : int32_t {
    RECORDER_PRIVATE_ERROR_TYPE_START = IRecorderEngineObs::ErrorType::RECORDER_ENGINE_ERROR_TYPE_MAX,
};

/**
 * @brief Recorder engine internel upward notification info type.
 */
enum RecorderPrivateInfoType : int32_t {
    RECORDER_PRIVATE_INFO_TYPE_START = IRecorderEngineObs::InfoType::RECORDER_ENGINE_INFO_TYPE_MAX,
};

/**
 * @brief Upward notification callback definition.
 * @param type: RecorderNotifierType
 * @param code: RecorderPrivateErrorType or RecorderPrivateInfoType
 * @param detail: Specific information about notification
 */
using RecorderNotifier = std::function<void(int32_t, int32_t, int32_t)>;
}
}
