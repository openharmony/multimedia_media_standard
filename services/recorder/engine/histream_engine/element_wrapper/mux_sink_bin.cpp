...
bool MuxSinkBin::DrainAll()
{
    ...
    GList *muxerAllSinkPad = gstMuxer_->sinkpads;
    for (GList *padNode = g_list_first(muxerAllSinkPad); padNode != nullptr; padNode = padNode->next) {
        ...
        if (!success) {
            MEDIA_LOGE("Send eos to muxer %{public}s's pad %{public}s failed",
                       GST_ELEMENT_NAME(gstMuxer_), GST_PAD_NAME(padNode->data));
            gst_event_unref(eos);
            return false;
        }
        MEDIA_LOGI("Send eos to muxer %{public}s's pad %{public}s finished",
                   GST_ELEMENT_NAME(gstMuxer_), GST_PAD_NAME(padNode->data));
    }
    gst_event_unref(eos);
...
int32_t MuxSinkBin::GetParameter(RecorderParam &recParam)
{
    (void)recParam;
    return ERR_OK;
}
...
GstPadProbeReturn MuxSinkBin::MuxerSinkPadProbe(const GstPad &pad, GstPadProbeInfo &info)
{
    ...
    MEDIA_LOGI("Capture EOS for pad %{public}s, pass it", GST_PAD_NAME(&pad));
    return GST_PAD_PROBE_PASS;
}
