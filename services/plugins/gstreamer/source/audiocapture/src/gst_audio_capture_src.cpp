...
static gboolean gst_audio_capture_src_negotiate(GstBaseSrc *bsrc);
...
static gboolean process_caps_info(GstAudioCaptureSrc *src)
{
    ...
    switch (channels) {
        case 1:
        {
            ...
            gst_audio_channel_positions_to_mask(positions, channels, FALSE, &channel_mask);
            break;
        }
        case 2:
        {
...
static GstFlowReturn gst_audio_capture_src_create(GstPushSrc *psrc, GstBuffer **outbuf)
{
    ...
    std::shared_ptr<AudioBuffer> audioBuffer = src->audio_capture->GetBuffer();
    g_return_val_if_fail(audioBuffer != nullptr, GST_FLOW_ERROR);
    gst_base_src_set_blocksize(GST_BASE_SRC_CAST(src), audioBuffer->dataLen);

    *outbuf = audioBuffer->gstBuffer;
    GST_BUFFER_DURATION(*outbuf) = audioBuffer->duration;
    GST_BUFFER_TIMESTAMP(*outbuf) = audioBuffer->timestamp;
    return GST_FLOW_OK;
}

static gboolean gst_audio_capture_src_negotiate(GstBaseSrc *bsrc)
{
    GstAudioCaptureSrc *src = GST_AUDIO_CAPTURE_SRC(bsrc);
