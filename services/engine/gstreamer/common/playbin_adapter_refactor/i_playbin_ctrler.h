...
public:
    ...
    virtual int32_t SetSource(const std::string &url) = 0; // sync
    virtual int32_t SetSource(const std::shared_ptr<GstAppsrcWrap> &appsrcWrap) = 0; // sync
    ...
    virtual int32_t SetRate(double rate) = 0; // async
    virtual double GetRate() = 0; // sync
};
} // namespace Media
} // namespace OHOS
