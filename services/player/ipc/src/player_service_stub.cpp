...
int32_t PlayerServiceStub::Seek(uint64_t mSeconds, int32_t mode)
{
    CHECK_AND_RETURN_RET_LOG(playerServer_ != nullptr, ERR_INVALID_OPERATION, "player server is nullptr");
    return playerServer_->Seek(mSeconds, mode);
}

int32_t PlayerServiceStub::GetCurrentTime(uint64_t &currentTime)
{
    CHECK_AND_RETURN_RET_LOG(playerServer_ != nullptr, ERR_INVALID_OPERATION, "player server is nullptr");
    return playerServer_->GetCurrentTime(currentTime);
}

int32_t PlayerServiceStub::GetDuration(uint64_t &duration)
{
    CHECK_AND_RETURN_RET_LOG(playerServer_ != nullptr, ERR_INVALID_OPERATION, "player server is nullptr");
    return playerServer_->GetDuration(duration);
}

int32_t PlayerServiceStub::SetPlaybackSpeed(int32_t mode)
{
    CHECK_AND_RETURN_RET_LOG(playerServer_ != nullptr, ERR_INVALID_OPERATION, "player server is nullptr");
    return playerServer_->SetPlaybackSpeed(mode);
}

int32_t PlayerServiceStub::GetPlaybackSpeed(int32_t &mode)
{
    CHECK_AND_RETURN_RET_LOG(playerServer_ != nullptr, ERR_INVALID_OPERATION, "player server is nullptr");
...
{
    uint64_t mSeconds = data.ReadUint64();
    int32_t mode = data.ReadInt32();
    int32_t ret = Seek(mSeconds, mode);
    reply.WriteInt32(ret);
    return ERR_OK;
}
...
{
    int32_t mode = data.ReadInt32();
    int32_t ret = SetPlaybackSpeed(mode);
    reply.WriteInt32(ret);
    return ERR_OK;
}
...
int32_t PlayerServiceStub::SetVideoSurface(MessageParcel &data, MessageParcel &reply)
{
    ...
    surface->SetUserData(SURFACE_FORMAT, format);
