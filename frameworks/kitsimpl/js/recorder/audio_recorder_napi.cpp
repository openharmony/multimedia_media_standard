...
napi_value AudioRecorderNapi::Prepare(napi_env env, napi_callback_info info)
{
    ...
    if (recorder->SetUri(env, args[0]) != ERR_OK) {
        recorder->SendErrorCallback(env, info, recorder->errorCallback_,
            CALLBACK_CONFIGURE_ERROR_CODE, CALLBACK_CONFIGURE_ERROR_TYPE);
        MEDIA_LOGD("SetUri fail");
        return undefinedResult;
    }
...
napi_value AudioRecorderNapi::On(napi_env env, napi_callback_info info)
{
    ...
    napi_value jsThis = nullptr;
    napi_status status = napi_get_cb_info(env, info, &argCount, args, &jsThis, nullptr);
    CHECK_AND_RETURN_RET_LOG(status == napi_ok && argCount >= MIN_REQUIRED_ARG_COUNT, nullptr, "invalid argument");
...
int32_t AudioRecorderNapi::CheckValidPath(std::string path)
{
    std::string realPath;
...
int32_t AudioRecorderNapi::SetUri(napi_env env, napi_value args)
{
    ...
    CHECK_AND_RETURN_RET_LOG(status == napi_ok && exist, ERR_INVALID_OPERATION, "can not find uri property");

    napi_value configItem = nullptr;
    status = napi_get_named_property(env, args, "uri", &configItem);
    CHECK_AND_RETURN_RET_LOG(status == napi_ok, ERR_INVALID_OPERATION, "can not get uri property");
    ...
    CHECK_AND_RETURN_RET_LOG(status == napi_ok, ERR_INVALID_OPERATION, "can not get uri content");
    CHECK_AND_RETURN_RET_LOG(nativeRecorder_ != nullptr, ERR_INVALID_OPERATION, "nativeRecorder_ no memory");
    ...
    if (uriPath.find(fileHead) != std::string::npos) {
        ...
        CHECK_AND_RETURN_RET(CheckValidPath(filePath) == ERR_OK, MSERR_INVALID_VAL);
        CHECK_AND_RETURN_RET(!filePath_.empty(), MSERR_INVALID_VAL);
        fdNum = open(filePath_.c_str(), O_CREAT | O_WRONLY, 0664);
        ...
        CHECK_AND_RETURN_RET(fdNum >= 0, ERR_INVALID_OPERATION);
        CHECK_AND_RETURN_RET(nativeRecorder_->SetOutputFile(fdNum) == ERR_OK, ERR_INVALID_OPERATION);
    } else {
