     ...
     maxDuration_(-1)
{
    ...
    if (sem_init(&sem, 0, 0) != 0) {
        MEDIA_FATAL_LOG("sem_init failed.");
    }
}
...
RecorderSink::~RecorderSink()
{
    ...
    threadRunning = false;
    if (threadId != 0) {
        MEDIA_INFO_LOG("message thread is still running, pthread_join called");
        sem_post(&sem);  // to wake incase blocked on wait
        ret = pthread_join(threadId, NULL);
        MEDIA_INFO_LOG("pthread_join returned, ret = %d", ret);
        threadId = 0;
    }
    MEDIA_INFO_LOG("Sem destroy called");
    sem_destroy(&sem);
}
...
{
    if (started_) {
        MEDIA_ERR_LOG("RecorderSink is started ,SetMaxDuration must be set before Prepare");
        return ERR_ILLEGAL_STATE;
    }
    if (duration <= 0) {
...
{
    if (started_) {
        MEDIA_ERR_LOG("RecorderSink is started , SetMaxFileSize must set before Prepare");
        return ERR_ILLEGAL_STATE;
    }
    if (size <= 0) {
...
int32_t RecorderSink::SendCallbackInfo(int32_t type, int32_t extra)
{
    ...
    switch (type) {
        case RecorderCallback::RECORDER_INFO_MAX_DURATION_APPROACHING:
        case RecorderCallback::RECORDER_INFO_MAX_FILESIZE_APPROACHING:
        case RecorderCallback::RECORDER_INFO_MAX_DURATION_REACHED:
        case RecorderCallback::RECORDER_INFO_MAX_FILESIZE_REACHED:
        case RecorderCallback::RECORDER_INFO_NEXT_OUTPUT_FILE_STARTED:
        case RecorderCallback::RECORDER_INFO_FILE_SPLIT_FINISHED:
        case RecorderCallback::RECORDER_INFO_FILE_START_TIME_MS:
        case RecorderCallback::RECORDER_INFO_NEXT_FILE_FD_NOT_SET:
...
int32_t RecorderSink::SendCallbackError(int32_t errorType, int32_t errorCode)
{
    ...
    switch (errorType) {
        case RecorderCallback::ERROR_CREATE_FILE_FAIL:
        case RecorderCallback::ERROR_WRITE_FILE_FAIL:
        case RecorderCallback::ERROR_CLOSE_FILE_FAIL:
        case RecorderCallback::ERROR_READ_DATA_ERROR:
        case RecorderCallback::ERROR_INTERNAL_OPERATION_FAIL:
        case RecorderCallback::ERROR_UNKNOWN:
...
int32_t RecorderSink::Start()
{
    ...
    if (recCallBack_ != nullptr && sinkCallback_ != nullptr) {
        pthread_create(&threadId, nullptr, MessageThread, (void *)(this));
        if (threadId == 0) {
            MEDIA_ERR_LOG("create message thread failed.");
            return -1;
        }
    }
...
{
    if (!started_) {
        MEDIA_INFO_LOG("RecorderSink is stopped or not started");
        return SUCCESS;
    }
