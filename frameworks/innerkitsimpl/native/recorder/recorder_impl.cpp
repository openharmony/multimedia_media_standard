...
const std::map<RecorderErrorType, std::string> RECORDER_ERRTYPE_INFOS = {
    {RECORDER_ERROR_CREATE_FILE_FAIL, "create file failed"},
    {RECORDER_ERROR_WRITE_FILE_FAIL, "write file failed"},
    {RECORDER_ERROR_INTERNAL, "internal error type"},
    {RECORDER_ERROR_SERVICE_DIED, "recorder service died"},
...
std::shared_ptr<Recorder> RecorderFactory::CreateRecorder()
{
    ...
    CHECK_AND_RETURN_RET_LOG(ret == ERR_OK, nullptr, "failed to init RecorderImpl");
    return impl;
}
...
int32_t RecorderImpl::Init()
{
    ...
    CHECK_AND_RETURN_RET_LOG(recorderService_ != nullptr, ERR_NO_INIT, "failed to create recorder service");
    return ERR_OK;
}
...
{
    CHECK_AND_RETURN_RET_LOG(recorderService_ != nullptr, ERR_INVALID_OPERATION, "recorder service does not exist..");
...
{
    CHECK_AND_RETURN_RET_LOG(callback != nullptr, ERR_INVALID_VALUE, "input callback is nullptr.");
    CHECK_AND_RETURN_RET_LOG(recorderService_ != nullptr, ERR_INVALID_OPERATION, "recorder service does not exist..");
...
{
    CHECK_AND_RETURN_RET_LOG(recorderService_ != nullptr, ERR_INVALID_OPERATION, "recorder service does not exist..");
    return recorderService_->Release();
}
